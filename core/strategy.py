...
def should_buy(price: float, day_open: float, x_percent: int) -> bool:
    """Return True when the hourly price is close enough to the day's open.

    A BUY is triggered if the current hourly price is at least ``(1 - X/100)``
    times the day's open.  For example, with ``day_open=100`` and ``X=2`` the
    threshold becomes ``98``.  Any price of ``98`` or above will satisfy the
    condition and we purchase ``X`` shares.
    """
    if day_open is None:
        return False
