...
def should_buy(price: float, day_open: float, x_percent: int) -> bool:
    """Return ``True`` when the hourly price is within ``X``% of day open.

    A BUY is triggered if the current hourly price is **greater than or equal**
    to ``(1 - X/100)`` times the day's open. For example, with ``day_open=100``
    and ``X=2`` the threshold becomes ``98``. Any price of ``98`` or above will
